    ...
    while (1) // loop of events
    {
        ...
        for (int i = 0; i < nfds; i++)
        {
            ...
            else if (evs[i].events & EPOLLIN)
            {
                ...
                std::span<const char> resp_view = buildResponse(requested_path.value().string());
                
                /* 发回并关闭连接 */
                ssize_t nwrite = write(fd, resp_view.data(), resp_view.size());
                if (nwrite < 0)
                {
                    perror("read failed...");
                }
                // ssize_t nwrite = write(fd, resp_view.data().c_str(), resp.size());
                close(fd);
                epoll_ctl(epfd, EPOLL_CTL_DEL, fd, nullptr);
            }
